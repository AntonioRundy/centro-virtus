...
const pillars = [
  { icon: User,  title: "Centrado no Aluno",       desc: "Cada aluno é único. O nosso ensino parte das necessidades e do perfil de aprendizagem de cada estudante.", iconBg: "bg-yellow-100", iconColor: "text-yellow-700", border: "hover:border-yellow-300" },
  { icon: Heart, title: "Atenção Individualizada",  desc: "Professores dedicados que conhecem cada aluno pelo nome, acompanham o seu progresso e investem no sucesso.", iconBg: "bg-green-100",  iconColor: "text-green-700",  border: "hover:border-green-300"  },
  { icon: Clock, title: "Respeito pelo Ritmo",      desc: "Avançamos no ritmo certo para cada aluno, consolidando conhecimentos antes de introduzir novos desafios.",    iconBg: "bg-blue-100",  iconColor: "text-blue-700",  border: "hover:border-blue-300"  },
];
    ...
    <section id="metodologia" className="section-gray py-20 lg:py-32">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <span className="inline-block text-yellow-700 text-sm font-semibold tracking-widest uppercase mb-3">Como trabalhamos</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-14">
          {pillars.map(({ icon: Icon, title, desc, iconBg, iconColor, border }) => (
            <div key={title} className={`card-lift bg-white rounded-2xl border border-gray-200 ${border} p-8 shadow-sm transition-all`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto">
          {modalities.map(({ icon: Icon, title, desc, tag, tagClass }) => (
            <div key={title} className="card-lift bg-white rounded-2xl border border-gray-200 shadow-sm p-8">
