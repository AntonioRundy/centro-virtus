...
const accents = ["bg-[#f4b400]", "bg-green-500", "bg-blue-500", "bg-purple-500"];
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {services.map((service, i) => (
            <button
              ...
              className="card-elevated card-lift group text-left rounded-2xl p-7 hover:border-[#f4b400] transition-all duration-300 cursor-pointer"
            >
              <div className="text-4xl mb-5">{service.icon}</div>
              <div className={`w-10 h-1 rounded-full ${accents[i % accents.length]} mb-4 transition-all duration-300 group-hover:w-16`} />
              <h3 className="font-bold text-lg mb-2" style={{ color: "#0f172a" }}>{service.title}</h3>
              <p className="text-sm leading-relaxed mb-4" style={{ color: "#334155" }}>{service.shortDesc}</p>
              <span className="inline-flex items-center gap-1 text-xs font-semibold group-hover:gap-2 transition-all" style={{ color: "#f4b400" }}>
